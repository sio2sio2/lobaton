...
Datos
*****
Los datos se facilitan en formato `GeoJSON`_ en que la primera *feature*
describe generales de los datos suministrados (p.e. qué nombre se corresponde
con cada código de puesto) y las restantes *features* son los datos centro a
centro.

.. todo:: Completar esta descripción.
   ...
   * Un objeto con las opciones de constitución del objeto. Estas opciones
     ...
     ``ors``
         ...
         * ``loading``: Función que construye un indicador para notar la carga
           de datos remotos. Si es ``true``, usa el indicador interna; y si
           ``false``, se prescindirá de indicador alguno. Valor predeterminado:
           ``true``.
...
.. js:attribute:: origen
   ...
   del origen está asociado al evento g.
...
.. js:method:: agregarCentros(datos)
   ...
   .. note:: El fin de la carga de datos está asociado al evento
      "*dataloaded*".
...
.. js:method:: geoCodificar(query)
   ...
   .. note:: La geocodificación tiene asociado el evento *addressset*.

.. js:method:: setOrigen(latlng)

   Establece el origen de los viajes en el punto pasado como argumento.

   .. note:: La obtención del origen tiene asociado el evento *originset*.
...
.. js:method:: setIsocronas(point)
   ...
   .. note:: La generación de las isocronas tiene asociado el evento
      *isochroneset*.
