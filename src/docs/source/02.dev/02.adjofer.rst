...
Localidades
===========
Para facilitar la petición de localidades el objeto incorpora una capa
``L.GeoJSON`` de Leaflet_:

.. js:attribute:: localidades

   Capa de tipo ``L.GeoJSON`` que almacena las marcas que representan
   localidades.

La capa, por defecto, no se añade al mapa. Cada una de las marcas de esta capa
es una marca ``L.Marker`` normal que añade métodos
:js:attr:`Marker.getData` y :js:attr:`Marker.changeData`, análogos al
de la clase de marca que define ``leafext.js``. La definición de este tipo
simple de marca se facilita a través del atributo:

.. js:attribute:: Localidad

   Tipo de marca para las localidades.

.. todo:: Este tipo de marca es muy simple y no soporta filtrado.

Además, hay definido el evento:
   ...
   g.on("requestset", e => {
      if(e.marker instanceof e.target.Centro) {
         ...
         if(!!e.newval !== !!e.oldval) {
            ...
            return; // Al cambiar el icono, se redibuja (sobra refresh)
         }
      }
      e.marker.refresh();
   });
