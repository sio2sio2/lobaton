...
#. La creación de una capa para el tratamiento de los datos en formato
   *GeoJSON*. En este caso se ha supuesto que los datos se obtuvieron
   ...
   .. seealso:: Vea cómo :ref:`tratar datos que no tenga formato GeoJSON
      <dev-map-no-geojson>`.
...
``updater``
   ...
   .. warning:: Para la mejora del rendimiento, no se pasan todos los parámetros
      ...
      de "*content*", y si no se pasa *numvac*, no se modifica el número
...
.. note:: Por supuesto, podemos seguir añadiendo opciones definidas para la
   ...
   caso de ``className``, ``iconSize`` e ``iconAnchor``. El valor de la primera
   opción es la causante de que en el |CSS| que definía la forma del icono, se
   hubiera usado la clase "*chupachups*".

Correcciones
============
El :dfn:`sistema de correcciones` permite alterar los datos iniciales de las
marcas según una serie de criterios predefinidos. En el ejemplo anterior, un
cirerio podría ser: *eliminar todas las adjudicaciones* que sean de un colectivo
...
.. Hablar de prepare en onEachFeature (o en layeradd).
